    ...
    if($row)
{
  ...
  $returnresult = array('status' => 0, 'message' => 'Token mismatch');
...
if($user_id =="" or $community_id=="" or $request_status=="" or $token=="" or  $alert_lat=="" 
  or $alert_lang==""){
die(json_encode(array("status"=>0, "message"=> "Input parameters are not found")));
...
die(json_encode(array("status"=>"false", "message"=> "Input parameters are not found")));
    ...
    if ($oldpassword!='' && $newpassword!='') {
        ...
        if ($result) {
           ...
           // $query1 = mysql_query($sql1);
            $res = json_encode(array('status' => 1, 'msg' => 'Password change successfully.'));
            print_r($res);
           
        }

         else {
            echo json_encode(array('status' => 0, 'msg' => 'Old password not exist.'));
        }
    } else {
        echo json_encode(array('status' => 0, 'msg' => 'Your detail is not complete.'));
    }
}
 else{
   $returnresult = array('status' => 0, 'message' => 'Token mismatch');
    ...
    if (isset($_REQUEST['email'])) {
        ...
        if ($result) {
            ...
            if (!mail($to, $subject, $msg, $headers)) {
                echo json_encode(array('status' => 0, 'msg' => 'Email not sent.'));
            } else {
                echo json_encode(array('status' => 1, 'msg' => 'Email sent successfully.'));
            }
        } else {
            echo json_encode(array('status' => 0, 'msg' => 'This email is not exist.'));
        }
    }
}
}
...
die(json_encode(array("status"=>0, "message"=> "Input parameters are not found")));
    ...
    if($row)
{
    $response = $this->user_model->get_comment_communitywise($token);
    $arr = array();
    ...
    if($row)
{
    ...
    $arr = array();
            ...
            $returnresult = array('status' => 1, 'message' => 'Record fount', 'comment' => $arr);
          $data = json_encode($returnresult);
              print_r($data);
}else{
         $returnresult = array('status' => 0, 'message' => 'Record not fount');
    ...
    $arr2 = array();
            ...
            // echo $community_id_my;
              ...
              $json_data = $this->user_model->get_community_emergency_contact($community_id_my);
               if($json_data==''){
            $returnresult = array('status' => 0, 'message' => 'No record found');
            }
            // elseif($json_data){
           ...
           $returnresult = array('status' => 1, 'message' => 'Record found','all_record' =>$arr2);
